...
export const Markdown = ({ source }: Props) => {
  ...
  const copyCode = (children: React.ReactNode) => {
    if (React.isValidElement(children)) {
      ...
      React.Children.forEach(children.props.children, (child) => {
        if (typeof child === 'string') {
          codeText += child
        } else if (React.isValidElement<{ children?: string }>(child)) {
      ...
      <div id='markdown'>
        ...
        <MDXRemote
          ...
          components={{
            ...
            p: ({ children }) => (
              <Text as='p' typography='text-xl-medium'>
                {children}
              </Text>
            ),
            ...
            img: ({ src, alt }) => (
              <span className='image-container'>
                <Image src={src ?? ''} alt={alt ?? ''} width={800} height={400} priority />
              </span>
            ),
